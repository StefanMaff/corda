...
public class FlowCookbookJava {
    ...
    public static class InitiatorFlow extends FlowLogic<Void> {
        ...
        public Void call() throws FlowException {
            ...
            // DOCSTART 30
            PartyAndCertificate otherIdentity = getServiceHub().getKeyManagementService().freshKeyAndCert(getServiceHub().getMyInfo().getLegalIdentityAndCert(), false);
            ...
            PartyAndCertificate otherIdentity2 = getServiceHub().getKeyManagementService().freshKeyAndCert(getServiceHub().getMyInfo().getLegalIdentityAndCert(), false);
            ...
            final Map<Party, AnonymousParty> identities = nodes.stream().collect(Collectors.toMap(PartyAndCertificate::getParty, PartyAndCertificate::toAnonymous));
